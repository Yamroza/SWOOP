...
names := namesarray('users', 'categories', 'transactions', 'offers', 'photos',
                       ...
                       'offers_history', 'comments_history', 'admins');
...
CREATE TABLE photos (
                        ...
                        link varchar(45) unique
);

-- table for users
CREATE TABLE users (
                       user_id NUMBER (4) GENERATED BY DEFAULT ON NULL AS IDENTITY START WITH 1,
                       login varchar2 (45) NOT NULL CONSTRAINT passes_pk PRIMARY KEY,
                       password varchar2 (40) NOT NULL,
                       name varchar(45),
                       surname varchar(45),
                       birthdate date,
                       accountcreationdate date,
                       profile_photo varchar(45) --NUMBER REFERENCES photos (photo_id)
);
...
CREATE TABLE admins (
                        ...
                        email varchar(45),
                        date_employed date,
                        login varchar2 (45) REFERENCES users (login)
);
...
CREATE TABLE voivodships
(
    voivodship_id NUMBER GENERATED BY DEFAULT AS IDENTITY START WITH 2 INCREMENT BY 2 PRIMARY KEY,
    name VARCHAR(30),
...
CREATE TABLE offers (
                        ...
                        for_sale number(1) not null check(for_sale in (0,1)),
                        price number(8,2),
                        seller varchar2(45) REFERENCES users(login),
                        offer_status number,
                        localisation varchar(30) REFERENCES cities(name),
                        photo varchar (45) REFERENCES photos(link)
);
...
CREATE TABLE offers_history (
                                offer_change_id int GENERATED BY DEFAULT AS IDENTITY not null CONSTRAINT offer_h_pk PRIMARY KEY,
                                offer_id NUMBER REFERENCES offers(offer_id),
                                name varchar(30) not null,
                                description varchar(3000),
                                category varchar(30) REFERENCES categories(name),
